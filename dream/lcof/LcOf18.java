...
public class LcOf18 {
    ...
    public static ListNode deleteNode(ListNode head, int val) {
        ...
        /*
         * 【注】当头结点是val的情况，其实没有删除，只是把头结点的指针指向下一节点，模拟删除，整个链表还是完整的
         */
        if (head.val == val) {
            head = head.next;
            return head;
        ...
        /*
         * 需要一个temp指针指向head节点，为什么呢？因为需要返回删除后的这个链表的从头到尾完整版本，head在操作之后就指向了被删除的节点的父节点了
         * 【另外】这里temp = head，仅仅只是指向了head指针所在的位置。假如后面有删除操作「head.next = head.next.next」
         * 对temp进行遍历，这个链表是少了一个节点的！！
         */
        ListNode temp = head;
