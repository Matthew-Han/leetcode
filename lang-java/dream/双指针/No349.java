...
public class No349 {
    ...
    public static int[] intersection(int[] nums1, int[] nums2) {
        ...
        // 将 2 个 set 集合都装载到 list 中
        list.addAll(set1);
        list.addAll(set2);
        // 将 list 中的元素装载到 map 中，value 是相同 key 的数量
        for (Integer integer : list) {
            map.put(integer, map.getOrDefault(integer, 0) + 1);
        }
        List<Integer> temp = new ArrayList<>();
        // map 中数量大于 2 的元素即为两个原数组相同的元素，也就是交集
        map.forEach((k, v) -> {
            if (v > 1) {
                temp.add(k);
            }
        ...
        /*
         * 因为不能确定上面不能确定交集元素的个数，所以无法直接定义数组容量大小在循环中赋值，只能先装到 temp 列表
         * 根据 temp 的长度再实例化一个结果int数组
         */
        int[] result = new int[temp.size()];
